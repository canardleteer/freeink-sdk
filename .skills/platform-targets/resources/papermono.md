...
1. **Current firmware (this checkout).** `BoardConfig::PAPER_MONO`,
   ...
   `platformio.sample.ini` `[env:papermono]`.
   `docs/display-driver-references.md` (primary: "M5 board implementation
   and on-glass measurements").
...
0x4F**. `FREEINK_FB_PSRAM` on. Insets 9/7/3/7 (starting). Sample env sets
`-DFREEINK_DEVICE_PAPERMONO=1`, `-DBOARD_HAS_PSRAM`,
`-DUSE_BLOCK_DEVICE_INTERFACE=1`.

## Does this SDK align with m5-docs?
...
**Controller / gray: not the product page, not the first cut.** This
tree's own table (`display-driver-references.md`) names **on-glass
measurements** as the primary driver source. `PaperMonoDriver` is SSD1677
+ 3-gray host LUTs. m5-docs say SSD1677 + **4-level**. The name now
matches the page; the waveform does not. `platformio.sample.ini` comments
say SSD1677 (after `41f2a7f`) but still say **1-bit SDMMC**;
`PAPER_MONO.sdmmc` is 4-bit from the first cut. A consumer INI may still
comment SSD1683.
...
| SDMMC comments | | `platformio.sample.ini` still says 1-bit | |
| INI controller comment | SSD1683 | sample.ini SSD1677; some consumers still SSD1683 | page SSD1677 |
...
Do not treat BMI270 / SX1262 / ST25R3916 as SDK features on this
checkout.
