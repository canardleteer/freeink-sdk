---
device: x4pro
device_flag: FREEINK_DEVICE_X4PRO
sdk_profile: XTEINK_X4_PRO
sdk_header: libs/hardware/BoardConfig/include/BoardConfig.h
shared_binary_envs: [x4pro]
mcu_family: s3
board_package: esp32-s3-devkitc1-n16r8
psram_in_ini: true
psram_on_silicon: true
fb_in_psram: false
sdmmc: true
block_device_interface: false
width: 800
height: 480
fb_bytes: 48000
controllers: [SSD1677, UC8179, UC8279]
grayscale: 4-level
viewable_insets: {top: 9, right: 7, bottom: 3, left: 7}
touch: GT911
multitouch: true
has_home_key: true
frontlight: pwm-warm
ui_scale: 1.2
ppi_note: touch-board uiScale 1.2
usb_detect: none
shared_pads:
  13: display-cs
caps: [TOUCH, FRONTLIGHT, WARMLIGHT, RTC]
---

# Xteink X4 Pro

ESP32-S3, distinct from the C3 X4. 800×480; controller varies by batch
(SSD1677 / UC8179 / UC8279, probed at boot). Those are three drivers
(`Ssd1677Driver`, `Uc8179Driver`, `Uc8279X4Driver`) — do not treat the two
UltraChip batches as one Half/GC path. X3's `Uc8279Driver` (UC8279d) is a
different state machine. VER LUT_VER 0x02 vs 0x68 selects UC8279 X4 AA
tables only. GT911 with capacitive home key and SDK multi-touch. Warm/cool
PWM frontlight, 1-bit SDMMC, CW2017 gauge, PCF8563-compatible RTC.
`uiScale` 1.2. Bezel insets 9/7/3/7. 8 MB PSRAM on silicon.

Own sample env (`x4pro`): `-DFREEINK_DEVICE_X4PRO=1` and
`-DBOARD_HAS_PSRAM`. `FREEINK_SD_SDMMC` auto-enables; the sample does **not**
set `-DUSE_BLOCK_DEVICE_INTERFACE` (consumers that mount SdFat on the SDMMC
block device still need it). Framebuffer stays in DRAM; PSRAM is on for
other heaps. Opt-in `-DFREEINK_X4PRO_FAST_DU_SHORTCUT` (SSD1677-batch only)
is not in the sample.
